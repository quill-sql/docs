...
In App.js, let's wrap our React app with an `AdminProvider`. We suggest putting the `AdminProvider` somewhere high in your app, above any component that might need to access BI Platform components. If you're using `QuillProvider` in the same react app, make sure it is a child of `AdminProvider`.
...
function App() {
  return (
    <AdminProvider
      queryEndpoint="api.yourserver.com/quill"
