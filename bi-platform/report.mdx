...
If you want to use an existing report as a starting point for a new one, open the report from your dashboard and click `Duplicate`. Quill creates a copy of the report in the same dashboard and section, suffixed with `(copy)`, preserving the chart type, columns, filters, pivots, sorts, limits, reference lines, and any associated report builder state.

The duplicated report is independent of the original, so you can rename it and edit its query, chart, or filters without affecting the source report.
