...
<CardGroup cols={2}>
  <Card
    ...
    icon="code"
    href="/bi-platform/report#with-sql-sqleditor"
  >
    Create a report with SQL
  </Card>
  <Card
    title="No Code (ReportBuilder)"
    icon="hammer"
    href="/bi-platform/report#no-code-reportbuilder"
  >
    Create a report from a user interface
  </Card>
</CardGroup>
...
When the row field of a pivot is a date column, you can choose how to bucket those dates with the **Date bucketing** dropdown:

- **Automatic** &mdash; bucket size is inferred from the active date range filter (for example, a multi-year range buckets by year, a multi-month range buckets by month).
- **Daily**, **Weekly**, **Monthly**, or **Yearly** &mdash; force every row to the selected interval, regardless of the filter range.

Use a fixed bucket when you want consistent groupings across different date filters (for example, always show monthly revenue). Leave it on **Automatic** when you want the granularity to follow the user's selected range.
