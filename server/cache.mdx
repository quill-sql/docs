...
const quill = new Quill({
	...
	cache: {
		...
		ttl: 3600, // optional: cache entry lifetime in seconds (default: 24h)
	},
});
```
...
## 4. Tune the cache lifetime (optional)

By default, cached query results live for 24 hours before Quill refreshes them.
Pass `ttl` (in seconds) to the cache config to override that window — use a
shorter TTL for data that changes often, or a longer one for slow-moving
reference data.
