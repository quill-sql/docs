...
<Tabs>
  <Tab title='Quill Cloud'>
    ...
    <Tip>
      ...
      have one. Requests from Quill will always come from `34.133.137.225`, `209.71.81.30/32`, or `209.71.101.146`
    </Tip>
...
To connect to Redis over TLS, set `cacheType` to `rediss`. The Node SDK opens a
TLS-encrypted connection but does not present a client certificate, so your
Redis server must accept connections without one.

<Warning>
	If your Redis server requires client certificate authentication, configure it
	with `tls-auth-clients no` (or `optional`) before enabling `rediss`.
	Otherwise the connection will be rejected during the TLS handshake.
</Warning>
