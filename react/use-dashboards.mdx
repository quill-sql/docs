...
function CustomDashboard({ name }: {name: string}) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-6 gap-6">
        {reports.map((report: any) => (
          <ReportCard key={report.id} reportId={report.id} name={report.name} />
        ))}
      </div>
    </div>
  );
...
<ParamField path="dashboards" type="Dashboard[] | null">
  ...
  <Expandable title="Dashboard type">
    ...
    <ResponseField name="reports" type="QuillReport[]" required>
      Reports currently associated with this dashboard.
    </ResponseField>
    <ResponseField name="tenantKeys" type="string[]">
      Dashboard owner tenant fields.
    </ResponseField>
    ...
    <ResponseField
      name="sectionOrder"
      type="{ _id: string; section: string; reportOrder: string[] }[]"
    >
      Section/report ordering metadata.
    </ResponseField>
  </Expandable>
</ParamField>
