...
<ParamField path="streamEndpoint" type="string">
  The url of the streaming endpoint on your self-hosted server. Used to stream
  responses from the Quill agent chat token-by-token. If omitted, the agent
  falls back to a single non-streamed response from `queryEndpoint`. Quill
  Cloud users do not need to set this.
</ParamField>
