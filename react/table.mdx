...
```jsx Using useQuill
import { QuillProvider, Table, useQuill } from "@quillsql/react";
...
function App() {
  ...
  return (
    <QuillProvider tenants={[2]} publicKey="6579031b3e41c378aa8180ec">
      <Table rows={report.data?.rows ?? []} columns={report.data?.columns ?? []} />
    </QuillProvider>
  );
}
```
...
<ParamField path="hideCSVDownloadButton" type="boolean">
