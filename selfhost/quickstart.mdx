...
if ($requestMethod === 'POST' && $endpoint === '/quill') {
    // Assuming you've authenticated the user and set their data in a session or token
    session_start();
    $authUser = $_SESSION['auth_user'] ?? null; // Example for session-based authentication
